...
jQuery(function ($) {
    ...
    function ajax_complete_callback(resp) {
        ...
        var ws_url = (window.location.origin + '/webssh').split(/\?|#/, 1)[0].replace('http', 'ws'),
        ...
        sock.onopen = function () {
            ...
            console.log("opts command:", url_opts_data.command)
            if (url_opts_data.command != undefined) {
                setTimeout(function () {
                ...
                setTimeout(function () {
                    // term.write(cmd)
                    sock.send(JSON.stringify({'data': 'clear' + '\r'}))
                }, 100)
            }
    ...
    function connect_without_options() {
        ...
        function ajax_post() {
            ...
            console.log('提交的data', data.values())
            $.ajax({
                url: url,
                type: 'post',
                data: data,
